...
class Auth extends MY_Controller {
    ...
    function index() {
        if ($this->session->userdata('logged_in')){
            redirect(base_url('subdistributor'));
        }
        ...
        if ($this->input->method(TRUE) === 'POST'){
            if ($this->authmodel->verify_validation()){
                ...
                if ($this->authmodel->check_login($username,$password)){
                    $user_data = $this->subdistributor->get_subdistributor($username);
    ...
    public function forget_password(){
        ...
        if($this->input->post('email')){
            ...
            $token = $this->subdistributor->get_forgot_password($email);
            ...
            if($token){
             ...
             $message = '<p>Hello </p><br> <br><p>Please <a href="'.base_url('subdistributor/auth/reset_password/'.$token).'">click here</a> to reset your password </p> <br><br> <p></p>';
                ...
                if($mail_res)
                {
                    ...
                    redirect('subdistributor/auth/forget_password');         
                }
...
public function reset_password($token){
        ...
        if($token){
            $response = $this->subdistributor->check_token($token);
            if($response){
                $data['token'] = $token;
            }else{
            ...
            if($this->input->post()){
                ...
                if($this->form_validation->run()){
                    ...
                    $update = $this->subdistributor->change_userdata($dt, $where);
